...
obj1={
   countries: [["Италия", "Рим", "Евразия", "Европа", -2, "итальянская лира", 
  ...
  {"1810":"Мексиканская война за независимость", "1536":"открытие коллегиума Санта-Крус де Тлателолько", "2006":"Революция кактусов"}
   ...
   outCountries: 
      function(a){
        function outArrayData(str,index){
 if(typeof(str)=="string")
    str="<tr><td> "+"&#1010"+(index+2)+" "+str+"</td><tr>";
  else
    str="<tr><td> "+"&#1010"+(index+2)+" "+str[0]+" &laquo"+str[1]+"&raquo "+"("+str[2]+")"+"</td><tr>";
 return str;
}
  
function makeTableFromCountry(country){
var gObj=country[6].map(outArrayData);
tab="<table width=100% margin=auto>\n";
tab +="<tr> <td colspan=2> <center> <h1>"+country[0]+" - "+country[1] + "</h1> </center> </td> </tr>\n";
tab+="<tr><td colspan=2> <table align=center width=75% margin=auto>\n";
for (var i=2;i<6;i++){
  tab+="<tr><td align=center width=50%>"+about[i]+"</td><td>"+country[i]+"</td></tr>\n";
}
tab+="</table></td></tr>\n";
tab+="<tr><td><br></td></tr>\n";
tab+="<tr>";
tab+="<td width=50% align=center valign=top><i>";
tab+=about[6];
tab+="</i></td>";
tab+="<td>\n";
tab+="<table>\n";
gObj.map(function(item){tab+=item;});
tab+="</table>";
tab+="</td>\n";
gObj = country[7].map(outArrayData);
tab+="<tr>";
tab+="<td width=50% align=center valign=top><i>";
tab+=about[7];
tab+="</i></td>";
...
tab+="</td>";
tab+="</tr>\n";
tab+="<tr><td colspan=2><center><b>";
tab+=about[8];
tab+="</b></center></tr></td>"
tab+="<tr>\n";
tab+="<table border=2px>\n";
tab+="<tr>";
for(var i in country[8])
    tab+="<td><b>"+i+"</b> -  "+country[8][i.toString()]+"</td>\n";
tab+="</tr>\n";
tab+="</table>";
tab+="</tr>";
tab+="</table>\n";  
...
countriesTable.forEach(function(info,i,arr){
  document.getElementById("body").innerHTML+=info+"<br>";
});
}
};
...
function Changes() {
   this.countries = [["Италия", "Рим", "Евразия", "Европа", -2, "итальянская лира", 
    ["Альпы", "Пальмария", "Апеннинский полуостров", "Комо", "Тальяменто"], 
    [["Джованни Верга", "Жизнь среди полей", 1880], ["Анна Виванти", "Цирцея", 1912], ["Джузеппе Понтиджа", "Рождённые дважды", 2000]],
  {"1882":"избирательная реформа", "1945":"антифашистское восстание, 18-28 апреля"}
  ],
  ...
  {"1810":"Мексиканская война за независимость", "1536":"открытие коллегиума Санта-Крус де Тлателолько", "2006":"Революция кактусов"}
   ...
   function CheckandDo(country){
       for(var i in country[8]){
            if (data==1){
				country[8][i] =country[8][i].toUpperCase();
			}
            else country[8][i] =country[8][i].toLowerCase();
    }
    }
   this.countries.forEach(function(info,i,arr){
      CheckandDo(info);
   });
 }
};